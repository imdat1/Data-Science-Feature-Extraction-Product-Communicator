...
# user_question = 'What laptops are there that have an i5 processor and at least 8GB of RAM?'
# smth= sql_chain.invoke({"features": features_string,"question": user_question})
# print(smth)
# output_string = smth.replace("```sql", "").replace("```", "")
# print(output_string)

def generate_sql_query_laptop_mistral(user_question):
    repo_id = "mistralai/Mistral-7B-Instruct-v0.3"
    llm = HuggingFaceEndpoint(
        repo_id=repo_id, max_length=128, temperature=0.1, token=HUGGINGFACEHUB_API_TOKEN
    )
...
def generate_sql_query_laptop_google(user_question):
    llm = ChatGoogleGenerativeAI(model="gemini-pro", temperature=0.1)
    sql_chain = (
    RunnablePassthrough.assign(schema=get_schema)
        | prompt
...
def generate_sql_query_laptop_anthropic(user_question):
    llm = ChatAnthropic(temperature=0, model_name="claude-3-sonnet-20240229")
    sql_chain = (
    RunnablePassthrough.assign(schema=get_schema)
        | prompt
